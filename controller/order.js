...
export async function getAllOrder(req, res) {
    try {
        const query = `SELECT id,customer_name,customer_email,customer_phone,customer_address,customer_city,customer_notes,product_name,product_image,product_newBox,product_price,sku,color, create_date FROM orders`
...
export async function upDateOrderById(req, res) {
    try {
        ...
        let orderData = await executeQuery(
            `SELECT id,customer_name,customer_email,customer_phone,customer_address,customer_city,customer_notes,product_name,product_image,product_newBox,product_price,sku,color,create_date FROM orders where id="${id}"`
        )
        let result = orderValidation(req.body)
        if (result.error && orderData.length > 0) {
            res.status(400).send(result.error.details[0])
        } else {
            const query = `UPDATE orders set customer_name="${customer_name}", customer_email="${customer_email}", customer_phone="${customer_phone}", customer_address="${customer_address}", customer_city="${customer_city}", customer_notes="${customer_notes}", product_name="${product_name}",product_image="${product_image}", product_newBox=${ckNewBox}, product_price=${ckPrice}, sku="${ckSku}", color="${ckColor}" WHERE id=${id}`
            let orderDada = await executeQuery(query)
            res.json(orderDada)
        }
    } catch (err) {
    ...
    try {
        ...
        if (result.error) {
            ...
            let orderDada = await executeQuery(
                `INSERT INTO orders (id,customer_name, customer_email,customer_phone,customer_address,customer_city,customer_notes,product_name,product_image,product_price,sku,color,product_newBox,create_date)
                VALUES(${id},"${customer_name}","${customer_email}","${customer_phone}","${customer_address}","${customer_city}","${customer_notes}","${product_name}","${product_image}",${ckPrice},"${sku}","${color}",${ckNewBox}, CURRENT_TIMESTAMP())`
            )
